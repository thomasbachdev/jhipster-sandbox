...
import fr.uga.service.RappelQueryService;
import fr.uga.service.RappelService;
...
public class RappelResource {
    ...
    private final RappelQueryService rappelQueryService;

    public RappelResource(RappelService rappelService, RappelRepository rappelRepository, RappelQueryService rappelQueryService) {
        this.rappelService = rappelService;
        this.rappelRepository = rappelRepository;
        this.rappelQueryService = rappelQueryService;
    }
     ...
     */
    @PostMapping("/rappels")
    public ResponseEntity<RappelDTO> createRappel(@Valid @RequestBody RappelDTO rappelDTO) throws URISyntaxException {
    ...
    public ResponseEntity<RappelDTO> updateRappel(
        ...
        @Valid @RequestBody RappelDTO rappelDTO
    ...
    public ResponseEntity<RappelDTO> partialUpdateRappel(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody RappelDTO rappelDTO
    ) throws URISyntaxException {
     ...
     */
    @GetMapping("/rappels")
    public ResponseEntity<List<RappelDTO>> getAllRappels(RappelCriteria criteria) {
        log.debug("REST request to get Rappels by criteria: {}", criteria);
        List<RappelDTO> entityList = rappelQueryService.findByCriteria(criteria);
        return ResponseEntity.ok().body(entityList);
    }

    /**
     * {@code GET  /rappels/count} : count all the rappels.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/rappels/count")
    public ResponseEntity<Long> countRappels(RappelCriteria criteria) {
        log.debug("REST request to count Rappels by criteria: {}", criteria);
        return ResponseEntity.ok().body(rappelQueryService.countByCriteria(criteria));
    }
